...
const Register = {
          ...
          <div class="reg-form">
            ...
            <div class="field">
              ...
              <div class="pills" id="r-pills">
                ${PREP_OPTIONS.map(t => `<button class="pill ${t===30?'on':''}" data-t="${t}" onclick="Register.setPrepTime(${t},this)">${t}분</button>`).join('')}
              </div>
            ...
            <div class="field" style="margin-top:var(--sp-5)">
              <label class="field-label">동반자 초대 (선택)</label>
              <button class="invite-btn" id="r-invite" onclick="Register.openCompanionModal()">
                <span>+ 카카오톡 동반자 선택</span><span>👥</span>
              </button>
              <div class="invite-tags" id="r-tags"></div>
            </div>
          </div>
        <!-- Submit -->
  ...
  openCompanionModal() {
    const items = COMPANIONS.map(c => {
      const sel = this.invited.find(x=>x.id===c.id);
      return `<div class="modal-item ${sel?'selected':''}" onclick="Register.toggleCompanion(${c.id},this)">
        <div class="modal-item-icon" style="background:linear-gradient(135deg,${c.color},${c.color}88);color:#fff;border-radius:50%">${c.emoji}</div>
        <span class="modal-item-text">${c.name}</span>
        <div class="modal-item-check">✓</div></div>`;
    }).join('');
    App.showModal('👥 동반자 초대', items + `<button class="btn btn-primary" style="margin-top:var(--sp-4)" onclick="App.closeModal();Register.updateTags()">✅ 선택 완료</button>`);
  },

  toggleCompanion(id, el) {
    const i = this.invited.findIndex(c=>c.id===id);
    if (i===-1) { const c=COMPANIONS.find(x=>x.id===id); if(c)this.invited.push(c); el.classList.add('selected'); }
    else { this.invited.splice(i,1); el.classList.remove('selected'); }
    U.haptic();
  },

  updateTags() {
    const c = U.$('#r-tags'); if(!c)return;
    c.innerHTML = this.invited.map(x=>`<div class="invite-tag">${x.emoji} ${x.name} <span class="x" onclick="Register.removeInvite(${x.id})">✕</span></div>`).join('');
    const btn = U.$('#r-invite span:first-child');
    if(btn) btn.textContent = this.invited.length ? `${this.invited.length}명 선택됨` : '+ 카카오톡 동반자 선택';
  },

  removeInvite(id) { this.invited=this.invited.filter(c=>c.id!==id); this.updateTags(); U.haptic(); },
