...
const State = {
  ...
  addSchedule(sched) {
    if (!sched.id) {
      sched.id = 'id_' + Date.now() + '_' + Math.floor(Math.random() * 1000);
    }
  ...
  cancelNativeAlarm(sched) {
    if (!sched) return;
    if (window.NativeApp && typeof window.NativeApp.cancelAlarm === 'function') {
      try {
        if (sched.id) {
          window.NativeApp.cancelAlarm(`sched_${sched.id}_prep`);
        }
        if (sched.timeline && sched.timeline.prepStart && sched.date) {
          const sDate = new Date(sched.date);
          const [hour, min] = sched.timeline.prepStart.split(':').map(Number);
          sDate.setHours(hour, min, 0, 0);
          window.NativeApp.cancelAlarm(sDate.getTime());
        }
        console.log(`[Alarm Cancelled] Sched ID: ${sched.id || 'N/A'}`);
      } catch (e) {
        console.error('Failed to cancel old alarm', e);
      }
    }
  },
  ...
  updateSchedule(idx, sched) {
    if (this.schedules[idx]) {
      if (!sched.id) {
        sched.id = this.schedules[idx].id || ('id_' + Date.now() + '_' + Math.floor(Math.random() * 1000));
      }
      this.cancelNativeAlarm(this.schedules[idx]); // 기존 알람 취소
    ...
    if (data) {
      try {
        ...
        this.schedules = parsed.filter(s => s && s.date).map(s => {
          ...
          if (!s.id) {
            s.id = 'id_' + (s.date ? new Date(s.date).getTime() : Date.now()) + '_' + Math.floor(Math.random() * 1000);
          }
          return s;
  ...
  calculateTimeline(idx) {
    ...
    const teeTotal = (isNaN(th) ? 7 : th) * 60 + (isNaN(tm) ? 0 : tm);
    const mannerTime = s.isDirectAlarm ? 0 : (s.isTestAlarm ? 1 : ((s.hasMeal && s.mealRestaurant) ? 30 : 40));
